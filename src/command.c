...
{
	while ((_dtoupper(*s1) == _dtoupper(*s2))&& (*s1)){s1++; s2++;};
if (*s1==*s2) return 0;
...

{
	...
	case CONFIG_ITEM_setPoint:
		...
		if (sysConfig.defaultProfile == SETPOINT_STATIC)
		{
			...
			commandprintf("SetPoint is set to %d.%d°C\n", setpointSet / DEGREE, (setpointSet % DEGREE * 10) / DEGREE);
		}
		else
...

{
	...
	if (sysConfig.defaultProfile <= MAX_PROFILES) commandprintf("%d\n", sysConfig.defaultProfile);
		...
		if (sysConfig.defaultProfile == SETPOINT_IDLE) commandprintf("IDLE\n");
		else
			if (sysConfig.defaultProfile == SETPOINT_STATIC) commandprintf("STATIC\n");
			else
				commandprintf("??????\n");
	...
	do
	{
		...
		do
		{
			commandprintf("    %d %s %3d°C in %3d seconds\n", profileStep + 1,
					profileGetStepname((sysConfig.profile[profileNum].step[profileStep].command & PROFILE_COMMAND_MASK) >> 28),
				...
				&& ((sysConfig.profile[profileNum].step[profileStep].command & PROFILE_COMMAND_MASK) != PROFILE_END));
...

{
	...
	while (logIteratorNext(&n))
	{
		...
		{
			if (((compareLog == 0) || (compareLog == currentLog)) && (currentLog != 0) && (!sysConfig.logOutputCSV))
