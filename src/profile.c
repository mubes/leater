...

{
	...
	// Step tag
	sprintf(constructString,"[%s %d/%d] ", sysConfig.profile[_condition.profile].name, _condition.profile + 1, _condition.stepNumber+1);

	if (_condition.stepNumber >= MAX_PROFILE_STEPS)
	{
		// Hey, we ended!
		sprintf(constructString,"%sEnds (%d seconds total runtime)",constructString, _condition.totalRuntime / mS);
		profileStop();
	}
	else
	{
		...
		sprintf(constructString,"%s%s ",constructString,
				_stepnames[(sysConfig.profile[_condition.profile].step[_condition.stepNumber].command & PROFILE_COMMAND_MASK)>>28]);

		switch (sysConfig.profile[_condition.profile].step[_condition.stepNumber].command & PROFILE_COMMAND_MASK)
		{
		// -----------------
		case PROFILE_FOREVER:
			// We are just doing to stay in this state until actively stopped
			stateChangeSetpoint(sysConfig.profile[_condition.profile].step[_condition.stepNumber].temp&PROFILE_TEMP_MASK);
			sprintf(constructString,"%sFOREVER stay at %d°C",constructString,(sysConfig.profile[_condition.profile].step[_condition.stepNumber].temp)&PROFILE_TEMP_MASK / DEGREE);
			break;

		// -----------------
		case PROFILE_JUMP:
			// We are going to move to another profile
			sprintf(constructString,"%sJUMP request to profile %d",constructString, sysConfig.profile[_condition.profile].step[_condition.stepNumber].otherProfile);
		...
		default:
			...
			sprintf(constructString,"%s %d°C in %d seconds (%d seconds elapsed)",constructString,
						((sysConfig.profile[_condition.profile].step[_condition.stepNumber].temp)&PROFILE_TEMP_MASK) / DEGREE,
			...
			_condition.remainingStepTime = sysConfig.profile[_condition.profile].step[_condition.stepNumber].time;
			_condition.stepType = sysConfig.profile[_condition.profile].step[_condition.stepNumber].command & PROFILE_COMMAND_MASK;

			// Store the intended final temperature for this step
			_condition.intendedFinal = sysConfig.profile[_condition.profile].step[_condition.stepNumber].temp&PROFILE_TEMP_MASK;

			// And increment either TEMP or TIME depending on step type
			if ((sysConfig.profile[_condition.profile].step[_condition.stepNumber].command & PROFILE_COMMAND_MASK) == PROFILE_REACHTEMP)
			{
			...
			else
			{
				...
				_condition.stepTemp = _dabs((((int32_t)_condition.commandedTemp-(int32_t)_condition.intendedFinal)*INC_INTERVAL))/_condition.remainingStepTime;
			}
...

{
	if (((_condition.remainingStepTime < INC_INTERVAL) && (_condition.stepType != PROFILE_REACHTEMP))
			|| ((sensorReturnReading() >= _condition.intendedFinal) && (_condition.dirPositive) && (_condition.stepType == PROFILE_REACHTEMP))
			|| ((sensorReturnReading() <= _condition.intendedFinal) && (!_condition.dirPositive) && (_condition.stepType == PROFILE_REACHTEMP)))
	{
...

{
	...
	if ((sysConfig.defaultProfile>0) && (sysConfig.defaultProfile <= MAX_PROFILES)) profileRun(sysConfig.defaultProfile-1);
	else
		profileStop();
}
// ============================================================================================
...

{
	...
	if (sensorReturnReading()!=TEMP_INVALID)
		sprintf(profileText,"%sS:%d.%d°C A:%d.%d°C H:%d%%)>", profileText, stateGetSetpoint() / DEGREE, ((stateGetSetpoint()*10) / DEGREE)%10,sensorReturnReading() / DEGREE, ((sensorReturnReading()*10) / DEGREE)%10,heaterGetPercentage());
	else
