...
uint32_t flag_test(uint32_t flag_to_check);			// Test for a specific flag, and reset it in the process
uint32_t flag_clear(void);							// See if there are any flags set at all
uint32_t flag_get(void);							// Get the current flagset, and reset them all in the process
const char *flag_getname(uint32_t flat_to_check);	// Return the name of the requested flag (for pretty printing)
