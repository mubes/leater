...
// ---------------------------------------------------
void logInitIterator(logIterator *n);						// Iterators cycle over the log data.  They are set to initial values here
logStateType logIteratorState(logIterator *n);				// Return current state of the log iterator
...
BOOL logGotoEnd(logIterator *n);							// Cycle though the log entries until we get to empty space

// Accessors for current value from log
uint32_t logVariable(logIterator *n);						// Get the next logged element from the iterator
const char *logVariableName(logIterator *n);				// Get the prettyprintable name of the next logged element from the iterator
uint32_t logValue(logIterator *n);							// Get the value from the iterator
uint32_t logScale(logIterator *n);							// Get the integer representing the scale for the reading
const char *logUnits(logIterator *n);						// Get the string representing the units for the variable

// Log manipulation
// ----------------
BOOL logNewLog(void);										// Create a new log at the end of the storage
BOOL logFlushLogs(void);									// Flush all logs from the store and create a new one
uint32_t logNumLogs(void);									// Return number of logs in the store

// Data entry
// ----------
BOOL logWrite(uint32_t variable, uint32_t value);			// Write a logged variable to the store...commit to underlying storage as necessary.
