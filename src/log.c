...

{
	...
	while ((bitsToRead--) && (n->state==LOG_OK) && (activeLog==n->currentLog)) n->variable=(n->variable<<1)|_unpump(n);
	...
	while ((bitsToRead--) && (n->state==LOG_OK) && (activeLog==n->currentLog)) n->value=(n->value<<1)|_unpump(n);
